...
{ 
	...
	aux.data = 0*3.14/180;
        pub_m11.publish(aux);
	aux.data = -27.666*3.14/180;
        pub_m12.publish(aux);
	aux.data = 0.619*3.14/180;        
	pub_m13.publish(aux);
	aux.data = -42.992*3.14/180;        
	pub_m14.publish(aux);
	aux.data = 15.326*3.14/180;
        pub_m15.publish(aux);
	aux.data = -0.619*3.14/180;
        pub_m16.publish(aux);
	aux.data = 0*3.14/180;
        pub_m21.publish(aux);
	aux.data = -14.746*3.14/180;
        pub_m22.publish(aux);
	aux.data = 5.2503*3.14/180;
        pub_m23.publish(aux);
	aux.data = 41.781*3.14/180;
        pub_m24.publish(aux);
	aux.data = -27.035*3.14/180;
        pub_m25.publish(aux);
	aux.data = -5.250*3.14/180;
        pub_m26.publish(aux);
	aux.data = 0*3.14/180;
        pub_m31.publish(aux);
	aux.data = 0*3.14/180;
...
/* ------------------------------------------------------------------*/ 
	...
	std::ifstream file("/home/pi/catkin_ws/src/test_node/bin/winkel.csv");
	std::string line;
